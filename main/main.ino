...
int tactLenght = 1500; //Length of the tact (in ms)
float maxPressDelay = 1; //NOTE: must be smaller or the same as tactLenght/NoteTiming (DONT FORGET TO ADD DYNAMIC DELAYS!!!) EDIT: Added dynamic delays and changed accuracy to percent
...
float songTimings[] = //assign a denominator for the tactLenght Value for every note, which is then used to determine its absolute Length, depending on the Length of one tact
{
...
float play(int songNotes[], float songTimings[], int songDirections[], int songLenght, int noteDelay, int tactLenght, float maxPressDelay) //takes the Notes, the Timings, the Directions, the Length of the Song(IN NOTES, NOT IN MS!!!), the Length of a tact (IN MS, NOT IN NOTES!!!) and the max button press delay (Ill probably replace that with a multiplier for the absolute Length of a note).
{
  ...
  for (int i = 0 ; i <= songLenght-1 ; i++)
  {
    ...
    setLEDs(songDirections, songLenght, i); //set the leds
    
    println("playing tone with pitch, direction, Length: " + String(currentNote) + "," + String(currentDirection) + "," + String(tactLenght/currentTiming));
    ...
    {
      currentMaxPressDelay = (maxPressDelay * (tactLenght / currentTiming));
    ...
    delay(tactLenght / currentTiming - currentPressDelay); //waits until the note is over, depending on the time spent in measurePressDelay
    noTone(SPEAKERPIN);
    delay(tactLenght/noteDelay); //adds a really smol delay after each note
  }
  noTone(SPEAKERPIN);
...
void setLEDs (int songDirections[], int songLenght, int initialIndex) //goes through every led, turns it off, and then turns it on if it finds a matching value at the according SongDirection index
{
  for(int i = 0; i <= MAXLEDPIN_0 - MINLEDPIN_0; i++)
    {
      digitalWrite(MINLEDPIN_0 + i, LOW);
      if(songLenght > initialIndex + i) //checking left notes
      {
...
int measurePressDelay(int button, float maxPressDelay) //doesnt really "measure". Just waits for a specified max time and returns how long the player needed
{
...
void setup() //some normal setup stuff (setting pins and serial initialization)
{
  ...
  println("avg. accuracy: " + String(play(songNotes, songTimings, songDirections,  sizeof(songNotes)/sizeof(songNotes[0]), noteDelay, tactLenght, maxPressDelay)) + " percent", true); //plays the song and prints the accuracy
}
